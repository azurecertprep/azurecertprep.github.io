...
:::tip AzureLoadBalancer service tag
You can use the `AzureLoadBalancer` service tag as an alternative to the literal IP `168.63.129.16`. Both are equivalent, but the service tag is the recommended, forward-compatible approach.
:::
...
Remove all resources created in this challenge to avoid ongoing charges.
