...
| Concept | Detail |
|---------|--------|
| Private DNS zone | Global resource providing name resolution within linked VNets |
| Registration VNet link | VNet linked with auto-registration; VM records created automatically |
| Resolution VNet link | VNet linked without registration; can query but does not auto-register |
| Auto-registration limit | A VNet can have auto-registration enabled on only 1 private DNS zone |
| Resolution link limit | A VNet can be linked to up to 1000 private DNS zones for resolution |
| Record set limit | 25,000 record sets per private DNS zone |
| Auto-registration scope | Only works for virtual machines (not load balancers, private endpoints, etc.) |
| SOA and NS records | Auto-created and system-managed; cannot be deleted |
| Privatelink zones | Special zones (e.g., `privatelink.blob.core.windows.net`) for private endpoints |
| DNS zone groups | Automatically manage A records in privatelink zones for private endpoints |
| Region independence | Private DNS zones are global; VNets from any region can be linked |
